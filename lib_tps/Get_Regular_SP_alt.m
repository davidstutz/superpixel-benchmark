function [ superpixel_region_label, superpixel_map] = Get_Regular_SP_alt( img, edge_map, H_width,W_width,gamma,sigma )
% Get the Regular Superpixel
    ...
    H_num = ceil(height/H_width);
    W_num = ceil(width/W_width);
    SP_num = H_num*W_num;
    r = round(gamma*min([round(width/(H_num));round(height/(W_num))]) -1);
    alpha = r/2;
    deta = sigma*r;
...
%     seed_w( seed_w < 1 ) = 1;
    ...
    [eh, ew, ec] = size(edge_map);

    edge_map_t = edge_map;
    edge_map = zeros(eh + r, ew + r);
    edge_map(1:eh, 1:ew) = edge_map_t;
    ...
    for i = 2:W_num
        for j = 2:H_num
            Region = edge_map( seed_h(i,j) - r +1: seed_h(i,j) + r, ...
                    seed_w(i,j) - r +1: seed_w(i,j) + r );
