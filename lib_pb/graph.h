...
/*
    ...
    it was later reimplemented by Vladimir Kolmogorov based on open 
    publications.
...
// NOTE: in UNIX you need to use -DNDEBUG preprocessor option to 
// supress assert's!!!
...
public:
    ...
    // Constructor. 
    // The first argument gives an estimate of the maximum number of nodes 
    // that can be added to the graph, and 
    // the second argument is an estimate of the maximum number of edges.
    // The last (optional) argument is the pointer to the function which 
    // will be called if an error occurs; 
    // an error message is passed to this function. 
    // If this argument is omitted, exit(1) will be called.
    //
    // IMPORTANT: It is possible to add more nodes to the graph than 
    // node_num_max (and node_num_max can be zero). 
    // However, if the count is exceeded, then 
    // the internal memory is reallocated (increased by 50%) which is expensive. 
    // Also, temporarily the amount of allocated memory would be more 
    // than twice than needed.  Similarly for edges.
    // If you wish to avoid this overhead, you can download version 2.2, 
    // where nodes and edges are stored in blocks.
    ...
    // If num>1, then several nodes are added, and node_id of the first one 
    // is returned.
    ...
    // segment the node 'i' belongs (Graph<captype,tcaptype,flowtype>::SOURCE 
    // or Graph<captype,tcaptype,flowtype>::SINK).
    //
...
public:
    ...
    ////////////////////////////////////////////////////////////////////////////
    // 2. Functions for getting pointers to arcs and reading graph structure. //
    //    NOTE: adding new arcs may invalidate these pointers                 //
    //    (if reallocation happens). So it's best not to add arcs while       //
    ...
    //   - It is not necessary to call mark_node() if the change is 
    //     ``not essential'',
    //     i.e. sign(trcap) is preserved for a node and zero/nonzero status 
    //     is preserved for an arc.
    //   - To check that you marked all necessary nodes, you can call 
    //     maxflow(false) after calling maxflow(true).
    //     If everything is correct, the two calls must return the same value 
    //     of flow. (Useful for debugging).

    void mark_node(node_id i);

    // If changed_list is not NULL while calling maxflow(), then the algorithm
    // keeps a list of nodes which could potentially have changed their 
    // segmentation label.
    // Nodes which are not in the list are guaranteed to keep their old 
    // segmentation label (SOURCE or SINK).
    //
    // Example usage:
    //
    ...
    //  - In the example above, the user may omit calls 
    //    g->remove_from_changed_list(i) and changed_list->Reset() 
    //    in a given iteration.
    //    Then during the next call to maxflow(true, &changed_list) 
    //    new nodes will be added to changed_list.
    //  - If the next call to maxflow() does not use option reuse_trees, 
    //    then calling remove_from_changed_list()
    //    is not necessary. ("changed_list->Reset()" or "delete changed_list" 
    //    should still be called, though).
...
private:
    ...
    struct node
    {
        ...
        node        *next;      // pointer to the next active node
                                //   (or to itself if it is the last node 
		  	        //   in the list)
    ...
    node  *nodes;
    node  *node_last; // node_last = nodes+node_num, 
    node  *node_max;  // node_max = nodes+node_num_max;

    arc   *arcs;
    arc   *arc_last;  // arc_last = arcs+2*edge_num, 
    arc   *arc_max;   // arc_max = arcs+2*edge_num_max;
...
template <typename captype, typename tcaptype, typename flowtype> 
inline void Graph<captype,tcaptype,flowtype>::add_edge
   (
   node_id _i, 
   node_id _j, 
   captype cap, 
   ...
   )
{
    ...
    // PC 09/01/09 -- No point adding a 0/0 edge!
    if ( cap == 0 && rev_cap == 0 ) return;
...
template <typename captype, typename tcaptype, typename flowtype> 
inline typename Graph<captype,tcaptype,flowtype>::arc* 
	Graph<captype,tcaptype,flowtype>::get_first_arc()
{
    return arcs;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
inline typename Graph<captype,tcaptype,flowtype>::arc* 
	Graph<captype,tcaptype,flowtype>::get_next_arc(arc* a) 
{
    return a + 1; 
}

template <typename captype, typename tcaptype, typename flowtype> 
inline void Graph<captype,tcaptype,flowtype>::get_arc_ends
   (
   arc* a, 
   node_id& i, 
   node_id& j
...
template <typename captype, typename tcaptype, typename flowtype> 
inline void Graph<captype,tcaptype,flowtype>::set_trcap
	(
	node_id i, 
	tcaptype trcap
	)
...
template <typename captype, typename tcaptype, typename flowtype> 
inline typename Graph<captype,tcaptype,flowtype>::
    termtype Graph<captype,tcaptype,flowtype>::what_segment
	(
	node_id i, 
